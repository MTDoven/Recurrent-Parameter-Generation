...
class DDIMSampler(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, x_t, c, steps=20, method="linear", eta=0.05, only_return_x_0=True, interval=1):
        if method == "linear":
            a = self.T // steps
...
class DiffusionLoss(nn.Module):
    ...
    def __init__(self, device=torch.device("cpu"), sample_mode="ddim"):
        super().__init__()
        self.device = device
